...
Then $R_j := \sum_{k \in t_{I_j}} r_{k,j}$ is the total runtime for all jobs assigned to instance $I_j$ and is called the *load* on $I_j$.
The maximum runtime across all instances or the *makespan* $M$ of $S$ is defined as:
...
In such cases, the runtimes are usually distributed according to a known probability distribution.
Each runtime can be distributed according to a different probability distribution.
In some cases, even the probability distribution is not known or has no closed form expression.
...
where $t_{I_j}$ is the index of the tasks in $\mathcal{T}$ assigned to instance $I_j$ by $\mathcal{S}$ and we maximize *expected* utility by averaging over all possible runtimes.

A major contribution of the current work is a method to find a feasible schedule with the lowest processing cost when runtimes are distributed according to an unknown probability distribution.
...
Finding an optimal schedule for a set of tasks requires the comparison of schedules to determine the *best* schedule.
This comparison requires that each schedule have an associated property (which we call score) that is determined in the same way and with reference to the same baseline for all schedules.
In this chapter, we develop the definition of such a property and show that it can be used to select the best schedule from a set of schedules.
...
This is a constraint imposed by the submitter of the tasks and leads to a definition of score where score = the probability of the schedule allowing all tasks to be completed by the deadline.
Thus the scoring function maps an schedule to a value in $(0,1)$ called the score.
...
Since all schedules are scored against the same deadline, schedule score can be used to compare candidate schedules and select the schedule with the highest probability of getting the benefit.

As mentioned in the previous chapter, we will use makespan as the optimization criterion to select the best schedule from a set of feasible schedules.
This chapter will focus on scoring schedules for a single instance and the next chapter will deal with scoring schedules when multiple identical instances are used in parallel.
