...
This section specifies the problem in detail while subsequent sections develop methods to solve the problem in an efficient manner.
...
Time taken to complete a task depends on the type of instance on which the task is processed; slower instances will take more time to process the same task compared to faster instances. Let $C_j$ be the cost per hour of instance $I_j$ (specified in dollars).
...
The maximum runtime across all instances or the *makespan* $\mathcal{M}$ of $\mathcal{S}$ is defined as:

\begin{equation}\label{eq:makespandef}
\mathcal{M} = \max_{j \in \mathcal{I}} {R_j}
\end{equation}

The makespan of a schedule represents the time taken by the schedule to complete all tasks and determines whether the tasks will be completed by the deadline.
Therefore a feasible schedule is one whose makespan $\mathcal{M}$ is less than the deadline $\mathcal{D}$.
...
Section \ref{SchedulingOverview} refers to the enumeration of feasible schedules and selection of the optimal schedule as separate steps.
In the context of minimizing makespan, the number of possible schedules is so large that it is not possible to enumerate all feasible schedules in a reasonable amount of time.
Since minimizing makespan increases the probability of completing the tasks by the deadline, we focus on trying to find the schedule with the smallest makespan while simultaneously attempting to find the schedule with the least cost.
Finding the schedule with the smallest makespan and least cost represents an *optimization problem*.

The problem of minimizing makespan for tasks with deterministic runtimes has been studied extensively and is considered to be NP-hard even for the simple case of scheduling an arbitrary number of tasks on 2 instances [@Garey1979].
This has led to the development of various heuristics to find, in a reasonable amount of time, a schedule that is 'close enough' to the global optimum.
One such heuristic is called Longest Processing Time First.
...
It has been shown to minimize makespan for tasks whose runtimes are deterministic and known in advance [@Pinedo2012].
