...
This section specifies the problem in detail and outlines a possible solution.
...
Let $\mathcal{S}$ be a schedule and let $J_j$ be the set of tasks assigned to instance $I_j$ by $\mathcal{S}$.
Then $R_j := \sum_{k \in J_j} r_{k,j}$ is the total runtime for all jobs assigned to instance $I_j$ and is called the *load* on $I_j$.
The maximum runtime across all instances or the *makespan* $M$ of $\mathcal{S}$ is defined as:

\begin{equation}\label{eq:def:makespan}
...
Therefore, a *feasible* schedule is one whose makespan $M$ is less than the deadline $D$.

Makespan depends on the number of tasks and the number of instances used to process tasks.
The number of tasks is an input to the problem, so the only parameter that can be varied is the number of instances used to process the tasks.
Makespan will be lowest when each task is processed in parallel on a separate instance and highest when all tasks are processed sequentially on the same instance.
In both cases, cost depends on the number and type of instances used and how long each instance was used.
...
Here each partition refers to a possible schedule.
A set of 10 tasks can be assigned to instances into $115,975$ ways with the number of instances varying from 1 to 10.
20 tasks result in $50 \times 10^{12}$ possible schedules on 1 to 20 instances and 100 tasks result in $~ 4 \times 10^{115}$ schedules on 1 to 100 instances.
Even with a small number of tasks, the number of possible schedules is too large to evaluate exhaustively; so it is not possible to enumerate the subset of feasible schedules.

To work around this, we combine the enumeration of feasible schedules and determination of the optimal schedule into a single step and use stochastic optimization methods to find the feasible schedule with the lowest processing cost.
This is a multi-criteria *optimization problem* where one criterion (feasibility) is binary - a schedule either completes the tasks by the deadline or it does not - and the other criterion (processing cost) is continuous with a lower bound at 0 and an upper bound represented by the cost of processing each task on a separate instance of the most expensive instance type.
It is important to note that a feasible schedule may not be found if the deadlines for the set of tasks are unrealistic.
Even if the deadlines are realistic and a feasible schedule does exist, the benefit associated with a set of tasks may be lower than the cost of processing the tasks and it is cheaper to not process the tasks than to process the tasks and incur a loss.

Since makespan is instance type-specific, we need to evaluate schedules on multiple instance types and select the instance type associated with a feasible schedule with the lowest processing cost.
...
To evaluate schedules, we adopt a *decision-theoretic* framework and *maximize utility* to select the best schedule.
...
where $U_{k,m}$ is the utility of a schedule using $m$ instances, each of type $k$, $B$ is the benefit derived from processing all tasks by the deadline $D$, $C_k$ is the cost per hour of an instance of type $k$ (fixed and known in advance), and $R_j$ is the total time used by instance $I_j$ of type $k$.
...
Since all schedules have the same benefit, this simplifies to:
...
It is not uncommon to come across problems where the runtime of a task is known only when the task completes processing.
In certain cases, even the form of the probability distribution is unknown or has no closed form expression.
In these cases, the distribution of task runtimes must be determined empirically.
