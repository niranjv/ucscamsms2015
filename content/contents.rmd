...
*Pre-emptive* schedules allow tasks to be interrupted while *non-pre-emptive schedules* do not allow pre-emption - once a task has started processing on a certain resource, it must complete processing on the same resource.
...
\nomenclature{$\mathcal{I}$}{Set of instances available to process tasks. Each instance is associated with a *type* and a cost $C$.}
\nomenclature{$\mathcal{P}$}{Set of instance types that can process tasks in $\mathcal{T}$}
...
The total runtime $R_j$ for all jobs assigned to instance $I_j$ is called the  *load* on instance $I_j$ and is defined as
...
A *feasible* schedule is defined as a schedule with $M \le D$.
...
*Bell number* [@mathworld:bellnumber] refers to the number of ways a set with $n$ elements can be partitioned into subsets such that each element is present in exactly one subset.
...
To work around this, we use *stochastic optimization* methods to find feasible schedules while simultaneously keeping track of their processing cost.
...
To evaluate feasible schedules, we adopt a *decision-theoretic* framework and *maximize utility* to select the best schedule.
The *utility* of a schedule using $m$ instances of type $k$ can be expressed as:
...
Task runtimes are assumed to follow unknown distributions that cannot be expressed in closed form and the distribution for each combination of instance type and task length can be different.
...
If the number of tasks or, equivalently, runtimes, is a large enough number, say, 100, then the Central Limit Theorem (CLT) will apply and $M$ will have a *Normal* distribution.
...
If the number of tasks is not large enough for the CLT to apply, then the distribution of $M$ is determined via bootstrap resampling [@Efron1993].
...
\label{validate:runtimes:deteministic}
When runtimes are deterministic and known in advance, applying the above method is straightforward. Table \ref{table:validate-1instance-deterministic} shows the details for a set of 5 tasks associated with a benefit of \$30 and a deadline of 25 hours. These tasks can run on 3 different instances types A, B \& C. The instance types vary in their cost and speed of processing and so do the runtimes of tasks on these instance types. Only B \& C result in feasible schedules with C having the maximum utility even though it has the highest cost per hour.
...
The actual runtimes of the tasks represent the "truth" and the optimal schedule can be determined as shown in Section \ref{validate:runtimes:deteministic}.

Figure \ref{fig:1instance-stochastic-runtimes-known-dist-100tasks} shows the results of using the above method to determine the optimal schedule for a simulated dataset when the true optimal schedule is known.
250 trials were run where each trial consisted of processing 100 tasks on 3 different instance types.
The runtimes of the tasks were distributed according to various probability distributions whose support is the positive real line.
Since runtimes were known, true makespan could be calculated.
Makespan was then calculated using the above method.
It predicted the makespan and optimal schedule correctly in the vast majority of cases and the number of incorrect predictions are well under the error tolerance of 5\%.
Black dots in Figure \ref{fig:1instance-stochastic-runtimes-known-dist-100tasks} are the true makespan values while the dotted red lines are the upper and lower bounds for the 95\% confidence interval for the predicted makespan of the optimal schedule.
In addition to predicting the makespans correctly, the method also correctly predicted the instance type that will maximize utility in all cases.
Figure \ref{fig:1instance-stochastic-runtimes-known-dist-200tasks} and \ref{fig:1instance-stochastic-runtimes-known-dist-500tasks} show the results when processing 200 and 500 tasks/instance repsectively on the same 3 instances types.
Instance types for the optimal schedule are also predicted correctly.

When the runtime distributions are unknown or do not have a closed form expression, we construct an empirical distribution as described in Section \ref{Data}. Results in this case are very similar and are not shown.

``` {r 1instance-stochastic-runtimes-known-dist-100tasks, echo=FALSE, message=FALSE, fig.cap='95% confidence interval for makespan using Normal approximation when 100 tasks are processed on a single instance. \\label{fig:1instance-stochastic-runtimes-known-dist-100tasks}'}
```

``` {r 1instance-stochastic-runtimes-known-dist-250tasks, echo=FALSE, message=FALSE, fig.cap='95% confidence interval for makespan using Normal approximation when 250 tasks are processed on a single instance. \\label{fig:1instance-stochastic-runtimes-known-dist-250tasks}'}
```

``` {r 1instance-stochastic-runtimes-known-dist-500tasks, echo=FALSE, message=FALSE, fig.cap='95% confidence interval for makespan using Normal approximation when 500 tasks are processed on a single instance. \\label{fig:1instance-stochastic-runtimes-known-dist-500tasks}'}
```

``` {r 1instance-stochastic-runtimes-known-dist-1000tasks, echo=FALSE, message=FALSE, fig.cap='95% confidence interval for makespan using Normal approximation when 1000 tasks are processed on a single instance. \\label{fig:1instance-stochastic-runtimes-known-dist-1000tasks}'}
```
...
Once again, we use a simulated data set where we know the optimal schedule (truth) and show that the above method correctly selects this schedule.

Figure \ref{fig:1instance-stochastic-runtimes-known-dist-10tasks} shows the 95% confidence interval
for makespan for jobs with 10 tasks. Figures \ref{fig:1instance-stochastic-runtimes-known-dist-25tasks},  \ref{fig:1instance-stochastic-runtimes-known-dist-50tasks} and \ref{fig:1instance-stochastic-runtimes-known-dist-75tasks} show the corresponding confidence interval for 25, 50 and 75  tasks respectively. 1000 boostrap samples were obtained for the makespan distribution in each case.

``` {r 1instance-stochastic-runtimes-known-dist-10tasks, echo=FALSE, message=FALSE, fig.cap='95% confidence interval for makespan using bootstrap re-sampling when 10 tasks are processed on a single instance. \\label{fig:1instance-stochastic-runtimes-known-dist-10tasks}'}
```

``` {r 1instance-stochastic-runtimes-known-dist-25tasks, echo=FALSE, message=FALSE, fig.cap='95% confidence interval for makespan using bootstrap re-sampling when 25 tasks are processed on a single instance. \\label{fig:1instance-stochastic-runtimes-known-dist-25tasks}'}
```

``` {r 1instance-stochastic-runtimes-known-dist-50tasks, echo=FALSE, message=FALSE, fig.cap='95% confidence interval for makespan using bootstrap re-sampling when 50 tasks are processed on a single instance. \\label{fig:1instance-stochastic-runtimes-known-dist-50tasks}'}
```

``` {r 1instance-stochastic-runtimes-known-dist-75tasks, echo=FALSE, message=FALSE, fig.cap='95% confidence interval for makespan using bootstrap re-sampling when 75 tasks are processed on a single instance. \\label{fig:1instance-stochastic-runtimes-known-dist-75tasks}'}
```
