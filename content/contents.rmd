...
The output is a scheduling assignment that maps tasks to resources in a certain order.
Scheduling problems are typically subject to various feasibility constraints that define a valid assignment.
...
Once a set of feasible assignments is available, various optimization criteria are used select the best assignment from this set.
...
An important characteristic of scheduling problems is that the number of feasible assignments is usually very large.
In such cases, it is not possible to find the globally optimal assignment in a reasonable amount of time.
...
Most approaches to solving scheduling problems involve some kind of local search and try to find, in a 'reasonable' amount of time, an assignment that is 'close enough' to the global optimum. Such approaches are called Polynomial Time Approximation Schemes (PTAS) and generate a solution in polynomial time that is within $(1-\epsilon)$ of the optimal solution (for maximization problems), where $\epsilon$ is any value greater than 0.

Methods used to solve scheduling problems depend on the type of problem being solved.
...
Examples of such criteria include minimizing the maximum completion time (makespan) of a set of tasks, minimizing the weighted completion time of a set of tasks where each task is assigned a weight proportional to its benefit, minimizing the maximum delay of a set of tasks, etc.

In this work, we focus on the problem of finding optimal non-pre-emptive schedules that minimize makespan while processing a set of tasks on multiple identical resources in parallel. All tasks have the same weight, have not undergone any pre-processing and are available for processing.
...
Let $I := {I_1, I_2, \dots I_m}$ be the set of resources (also known as instances in this report) available to process tasks in $\mathcal{T}$.
...
We make the following additional assumptions so that the problem is well-defined:
...
This chapter will focus on determining makespan for a single instance and the next chapter will deal with determining makespan when multiple identical instances are used in parallel.
...
So tasks can be processed on this instance in an arbitrary order and the action space here is the set of instance types available in the data center.
The corresponding action is the selection of the instance that maximizes the expected utility function [ref to eq in Problem formulation section].
