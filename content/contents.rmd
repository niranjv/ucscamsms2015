...
* **Processor types:**
Each Amazon data center has hundreds of thousands of virtual machines  (also know as instances) (ref to EC2 website) running on physical hardware.
Each instance is assigned resources like processor type and speed, memory, local disk space, network speed, etc. and the combination of these resources is called a configuration (also known as instance type).
Examples of instance types include compute-optimized instances with faster, more powerful processors, memory-optimized instances with large amounts of memory, storage-optimized instances with large amounts of local disk space, etc.
There are dozens of such instance types and thousands of instances of the same type in each data center.
These instance types represent the action space with selection of an instance type to process input types being the corresponding action.
In this report, we assume that tasks are constrained only by processor type and speed and not by any other resource.
...
If these properties were easy to quantify, then runtime would be deterministic function of length and the properties.
...
Runtime for a task depends on the length of the task and the instance type on which the task is processed.
Distributions for task runtimes do not follow any known distribution and cannot be expressed in closed form.
Further, each task length can follow a potentially different unknown distribution.
So runtime distributions for a given task length and instance type are determined empirically by running multiple tasks of the same length on the instance type and recording the actual runtime for each task.
These empirically determined runtimes form the *training set* for the algorithm and are used to predict runtimes for future tasks with the same length.
...
Finding an optimal assignment requires the comparison of assignments to determine the *best* assignment.
...
Thus score is a function that maps an assignment to a value in $(0,1)$.
...
Since all assignments are scored against the same deadline, assignment score can be used to compare assignments and select the best assignment (i.e., assignment with the highest score).

The total time taken for a set of tasks to finish processing is called *Makespan*.
This chapter will focus on determining makespan for a single processor and the next chapter will deal with determining makespan when multiple identical processors are used in parallel.

For a single processor, makespan is the sum of actual runtimes of each task in the input set of tasks and is invariant to the order in which the tasks are processed.
So tasks can be processed on this processor in an arbitrary order and the action space here is the set of all types of processors that will complete the tasks by the deadline.
The corresponding action is to select the processor that maximizes the expected utility function (ref to eq in Intro).

Since task runtimes are not deterministic, but random variables following separate unknown distributions, makespan for a single processor is also a random variable.
The nature of the makespan distribution depends on the number of tasks whose runtimes are being summed.
...
Validation for scoring assignments on a single processor when there are < 50 tasks/processor.
