...
This chapter focuses on selecting optimal feasible schedules when processing tasks on a single resource while the next chapter deals with the case of processing tasks on multiple identical resources in parallel.
...
Since the number of instances is fixed at 1, the action space is reduced to the set of instance types that result in feasible schedules and the action is the selection of the instance type that results in a feasible schedule and maximizes (expected) utility.
...
Utility is calculated in this manner for each instance type in the action space.
...
Since $M$ is the sum of runtimes, if runtimes are stochastic, then $M$ is itself a random variable with an associated probability distribution.
This distribution of $M$ depends on the number of runtimes that are being summed.
Tasks in $\mathcal{T}$ are assumed to be independent and represent a random sample from the population of tasks.
The same therefore applies to the runtimes of these tasks.
...
If the number of tasks is not large enough for the CLT to apply, then the distribution of $M$ is determined via bootstrap resampling [ref to bootstrap].
A value is randomly sampled from the distribution of runtimes of each of the input tasks.
These values are then summed to give a value for makespan and the value is recorded.
...
If the distribution of runtimes is unknown or does not have a closed form expression, a value must be sampled from the empirical distribution for runtime.
...
This set of runtimes represents the empirical runtime distribution for a task with length $l$ when processed on an instance of type $k$.
